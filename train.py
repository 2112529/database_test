...
def get_base_model(model_name, image_size):
    ...
    if model_name == 'mobilenet0.75':
        base_model = MobileNet(include_top=False, weights='imagenet', alpha=0.75,
                               input_shape=(image_size, image_size, 3))
    if model_name == 'mobilenet1.0':
        base_model = MobileNet(include_top=False, weights='imagenet', alpha=1.0,
                               input_shape=(image_size, image_size, 3))
    if model_name == 'mobilenetv2':
        base_model = MobileNetV2(include_top=False, weights='imagenet', alpha=1.0,
                                 input_shape=(image_size, image_size, 3))
...
def train_model(model, dataset, image_size, batch_size, save_name, lr1, lr2, Epochs1, Epochs2):
    dataParam = {'messidor': [957, 243, 2, './data/messidor/train', './data/messidor/test'],
                 ...
                 'DDR': [9851, 2503, 5, './DDR/train', './DDR/valid']}  # 6119
...
history = train_model(parallel_model, 'DDR', image_size, batch_size, 'densenet121', lr1, lr2, 1,1)
